...
export default function Details ({ student, comments, newComment, handleChange, handleSubmit }){
    ...
    return (
     <>
        ...
        <div>
        {/* Ternary to show/hide details when show details button is clicked*/}
        {
            showDetails 
            ?
            (
            <>
                <section className="details" id={student} key={student.id}>
                    {/* Displays codewar scores and adds color styling based on percentage */}
                    <div className="codewars">
                        ...
                        <p><span>Goal: </span>{student.codewars.goal.total}</p>
                            ...
                            {percent > 50 && percent < 100 && <p><span>Percent of goal achieved: </span><span className='yellow'>{percent}%</span></p>}
                            {percent <= 50 && <p><span>Percent of goal achieved: </span><span style={{color: "red"}}>{percent}%</span></p>}
                    </div>
                    {/* Displays scores and assigns a className value of true/false for CSS color styling */}
                    ...
                    {/* Uses React-icons to show a check or X if certification is true/false */}
